...
# ========== 單一 Epoch 訓練 ==========
def train_one_epoch(G, D, loader, criterion_bce, criterion_cls, optim_G, optim_D):
    G.train(); D.train()
    ...
    for real_seq, real_label in loader:
        ...
        fake_labels = torch.bernoulli(torch.full((real_seq.size(0),), 0.5)).long().to(device)
        ...
        gen_out, gen_cls_out = D(gen_seq[:, -1, :])
    ...
    for rnd in range(num_rounds):
        ...
        for i in range(len(dataset_paths)):
            ...
            for _ in range(local_epochs):
                loss_G, loss_D = train_one_epoch(G, D, local_loaders[i], criterion_bce, criterion_cls, optim_G, optim_D)
            D.eval()
