...
def apply_adasyn(X, y, ratio=ADASYN_RATIO, random_state=42):
    sm = ADASYN(sampling_strategy=ratio, random_state=random_state, n_neighbors=5, n_jobs=-1)
...
def main():
    ...
    X_bal, y_bal = apply_adasyn(X_train, y_train)
    print(f"ADASYN applied: {len(X_bal)} samples (original: {len(X_train)})")
    ...
    stack_model = StackingClassifier(
        ...
        final_estimator=LGBMClassifier(n_estimators=100, learning_rate=0.05),
        passthrough=True,
        n_jobs=-1
    )
