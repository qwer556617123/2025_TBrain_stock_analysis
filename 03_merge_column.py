...
# 設定 chunksize
chunk_size = 10000  # 您可以根據您的記憶體情況調整這個值

# --- 分批讀取和處理數據，擬合 StandardScaler ---
...
for chunk in pd.read_csv('training_filtered_merged.csv', chunksize=chunk_size):
    y_chunk = chunk[target_column]
    X_chunk = chunk.drop(target_column, axis=1)
    X_chunk = X_chunk.drop(id_column, axis=1, errors='ignore')  # 使用 errors='ignore' 處理第一次讀取時可能沒有 ID 的情況

    current_numerical_features = X_chunk.select_dtypes(include=np.number).columns.tolist()

    if first_chunk:
        numerical_features = current_numerical_features
        first_chunk = False
    else:
        # 確保所有 chunk 的數值型特徵一致
        if set(numerical_features) != set(current_numerical_features):
            raise ValueError("不同 chunk 的數值型特徵不一致！")

    # 對當前 chunk 的數值型特徵進行 partial_fit
    scaler.partial_fit(X_chunk[numerical_features])
...
for chunk in pd.read_csv('training_filtered_merged.csv', chunksize=chunk_size):
    y_chunk = chunk[target_column]
    X_chunk = chunk.drop(target_column, axis=1)
    X_chunk = X_chunk.drop(id_column, axis=1, errors='ignore')

    # 標準化數值型特徵
    X_chunk[numerical_features] = scaler.transform(X_chunk[numerical_features])

    all_X.append(X_chunk)
    all_y.append(y_chunk)

# 將所有 chunk 合併為一個 DataFrame
X = pd.concat(all_X)
y = pd.concat(all_y)
